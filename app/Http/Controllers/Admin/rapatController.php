    ...
    public function save()
    {
        ...
        $messages=[
            'start_tgl_rapat.required'=>config('constants.ERROR_JML_WAJIB'),
             ...
             'status_pelaksana.required'=>config('constants.ERROR_JML_WAJIB'),
        ...
        $internal->PIC =Input::get('PIC');
        if (!empty(Input::get('tempat_rapat'))) {
        $internal->tempat_rapat =Input::get('tempat_rapat');

        }
        $internal->status_pelaksana =Input::get('status_pelaksana');
        ...
        $internal->status_fasilitator =Input::get('status_fasilitator');
        ...
        $internal->fasilitator =$dataF;

        $internal->pj_rapat =$dataPelaksana;
        $internal->phari =$tambahHari;
    ...
    if (!empty($infant_eselon) OR !empty($undang_eselon)) {
        ...
        if (!empty($tambahHari)) {
            for ($i=1; $i<=$tambahHari ; $i++) {
                ...
                $internal =new RP;
                $internal->agenda_rapat =Input::get('agenda_rapat');
                $internal->start_tgl_rapat =$startTambah;
                $internal->end_tgl_rapat =$endTambah;
                $internal->status_id_rapat =$last_id;
                $internal->status_ruangan_rapat =Input::get('status_ruangan_rapat');
                $internal->PIC =Input::get('PIC');
                if (!empty(Input::get('tempat_rapat'))) {
                $internal->tempat_rapat =Input::get('tempat_rapat');
                ...
                $last_idTambah = $internal->id_rapat;
                //Insert Infant Nama Bebas (No Db)
                $name_infant = Input::get('name_infant');
                if (!empty($name_infant)) {
                    foreach ($name_infant as $key => $value) {
